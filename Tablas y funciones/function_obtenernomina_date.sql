-- Function: obtenernomina(date)

-- DROP FUNCTION obtenernomina(date);

CREATE OR REPLACE FUNCTION obtenernomina(date)
  RETURNS SETOF datonomina AS
$BODY$
DECLARE

	dComienza  ALIAS FOR $1;
	dFinal		DATE;
	...
	SELECT date_trunc('month', dComienza::Date) + interval '1 month - 1 day' INTO dFinal;
	
	FOR iIdEmp IN 
	SELECT idempleado FROM movimientos WHERE fechamovimiento between dComienza and dFinal GROUP BY idempleado ORDER BY idempleado
	LOOP
		...
		FOR mov IN
		SELECT  puesto, puestocubrio, cantidadentregas FROM movimientos WHERE idempleado = iIdEmp and fechamovimiento between dComienza and dFinal 
		LOOP
